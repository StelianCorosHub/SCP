...
void Model::draw(const V3D &color,
                 const glm::mat4 &transform, double alpha) const {
    if (ShaderFactory::activeShader() == nullptr){
        printf("cannot draw model because no shader is active.");
        return;
    }
    ShaderFactory::activeShader()->use();
    ShaderFactory::activeShader()->setMat4("model", transform);
    for (unsigned int i = 0; i < meshes.size(); i++) {
        meshes[i].draw(color, alpha);
    }
}

void Model::setDisplayTexturesFlag(bool showTextures) {
    for (unsigned int i = 0; i < meshes.size(); i++)
        meshes[i].showTextures = showTextures;
}

void Model::draw(const V3D &color, double alpha) const {
    if (ShaderFactory::activeShader() == nullptr){
        printf("cannot draw model because no shader is active.");
        return;
    }
    ShaderFactory::activeShader()->use();
    ShaderFactory::activeShader()->setMat4("model", getTransform());
    for (unsigned int i = 0; i < meshes.size(); i++) {
        meshes[i].draw(color, alpha);
    }
}

void Model::draw(double alpha) const {
    if (ShaderFactory::activeShader() == nullptr){
        printf("cannot draw model because no shader is active.");
        return;
    }
