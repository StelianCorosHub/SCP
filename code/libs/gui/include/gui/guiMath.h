...
inline glm::quat toGLM(const Quaternion& q) {return glm::quat(q.x(), q.y(), q.z(), q.w()); }

inline Quaternion toQuaternion(const glm::quat& q) {return Quaternion(q.w, q.x, q.y, q.z);}
...
inline void setTransformFromWidgets(const glm::mat4 &cameraView,
    ...
    ImGuizmo::RecomposeMatrixFromComponents(
        matrixTranslation, matrixRotation, matrixScale, glm::value_ptr(matrix));
    ...
    /*
        if (mCurrentGizmoOperation != ImGuizmo::SCALE)
        {
            if (ImGui::RadioButton("Local", mCurrentGizmoMode ==
       ImGuizmo::LOCAL)) mCurrentGizmoMode = ImGuizmo::LOCAL; ImGui::SameLine();
            if (ImGui::RadioButton("World", mCurrentGizmoMode ==
       ImGuizmo::WORLD)) mCurrentGizmoMode = ImGuizmo::WORLD;
        }
    */
...
inline void setTransformFromWidgets(const glm::mat4 &cameraView,
                                    const glm::mat4 &cameraProjection,
                                    V3D &scale, Quaternion &orientation,
                                    P3D &position) {
    ...
    //    orientation = Quaternion(AngleAxisd(RAD(matrixRotation[0]), V3D(1, 0,
    //    0))) *
    //                  Quaternion(AngleAxisd(RAD(matrixRotation[1]), V3D(0, 1,
    //                  0))) * Quaternion(AngleAxisd(RAD(matrixRotation[2]),
    //                  V3D(0, 0, 1)));

    // don't understand why it's this order. In the reconstruct matrix method,
    // it seems to be the other way around...
    orientation = Quaternion(AngleAxis(RAD(matrixRotation[2]), V3D(0, 0, 1))) *
                  Quaternion(AngleAxis(RAD(matrixRotation[1]), V3D(0, 1, 0))) *
                  Quaternion(AngleAxis(RAD(matrixRotation[0]), V3D(1, 0, 0)));
}
