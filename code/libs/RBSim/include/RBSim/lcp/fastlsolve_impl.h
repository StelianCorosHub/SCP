...
void solveL1Straight (const double *L, double *B, unsigned rowCount, unsigned rowSkip)
{
    assert(rowCount != 0);
    ...
    for (; goForLoopX4; subsequentPass = true, goForLoopX4 = (blockStartRow += 4) <= loopX4LastRow) 
    {
        ...
        {
            ptrLElement = L + rowSkip/* + blockStartRow * rowSkip*/; assert(blockStartRow == 0);
            ptrBElement = B;
            /* set the Z matrix to 0 */
            Z11 = 0; Z21 = 0; Z31 = 0; Z41 = 0;
        }
        ...
        {
            double p2 = ptrLElement[0];
            Y21 = ptrBElement[1 * b_stride] - Z21 - p2 * Y11;
            ptrBElement[1 * b_stride] = Y21;
        }
        ptrLElement += rowSkip;
        {
            double p3 = ptrLElement[0];
            double p3_1 = ptrLElement[1];
            Y31 = ptrBElement[2 * b_stride] - Z31 - p3 * Y11 - p3_1 * Y21;
            ptrBElement[2 * b_stride] = Y31;
        }
        {
            double p4 = ptrLElement[rowSkip];
            double p4_1 = ptrLElement[1 + rowSkip];
            double p4_2 = ptrLElement[2 + rowSkip];
    ...
    for (; !subsequentPass || blockStartRow < rowCount; subsequentPass = true, ++blockStartRow) 
    {
        ...
        if (subsequentPass)
        {
            ...
            const double *ptrLElement = L + blockStartRow * rowSkip;
            ...
            /* finish computing the X(i) block */
            double Y11 = ptrBElement[0 * b_stride] - (Z11 + Z12);
            ptrBElement[0 * b_stride] = Y11;
        }
    }
}
