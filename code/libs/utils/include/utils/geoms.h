...
public:
    ...
    Segment getClosestSegmentToLine(const Line& other) const {
        double c_a = V3D(this->p, other.p).dot(this->v);
        double c_b = V3D(this->p, other.p).dot(other.v);
        double vavb = this->v.dot(other.v);
        double denom = 1 - vavb * vavb;

        double tb = 0;
        if (denom > 1e-8) tb = (-c_b + vavb * c_a) / denom;
        double ta = c_a + vavb * tb;

        return Segment(this->p + this->v * ta, other.p + other.v * tb);
    }
};
...
public:
    ...
    double getDistanceToPoint(const P3D& p){
        P3D cp;
        return getDistanceToPoint(p, &cp);
    }
    ...
    P3D intersectWith(const Plane& plane) {
        P3D p;
        if (getDistanceToPlane(plane, &p) > 0)
            return P3D(DBL_MAX, DBL_MAX, DBL_MAX);
        return p;
    }
};
...
public:
    ...
    //orientation of box
    Quaternion q;
    //dimensions of the box (length, height, width);
    V3D dims = V3D(1,1,1);

    Box3D() {}

    Box3D(const P3D& p, const Quaternion& q, const V3D& dims) {
        this->p = p;
        this->q = q;
        this->dims = dims;
    }
