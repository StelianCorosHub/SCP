...
public:
    ...
    void restart() { begin = std::chrono::high_resolution_clock::now(); }
            ...
            while (duration_tmp.count() < t) {
                ...
                std::this_thread::sleep_for(
                    std::chrono::duration<double>(std::chrono::milliseconds(
                        1)));  //Sleep for a bit (too inaccurate to be used on windows)
#endif  // WIN32
            }
        } else {
            std::this_thread::sleep_for(std::chrono::duration<double>(
                std::chrono::milliseconds((long long)(t * 1000.0))));
        }
    }
